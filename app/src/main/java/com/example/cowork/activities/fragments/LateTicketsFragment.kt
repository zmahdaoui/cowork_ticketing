...
class LateTicketsFragment : Fragment(), View.OnClickListener  {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val view = inflater.inflate(R.layout.fragment_late_tickets,  container, false)!!

        recyclerView = view.findViewById(R.id.late_tickets)
    ...
    private fun loadData(id: Int) {
        myCompositeDisposable.add(
            RetrofitClient.instance.getProLateTicket(id)
    ...
    private fun loadData2(id: Int) {
        myCompositeDisposable.add(
            RetrofitClient.instance.getClientLateTicket(id)
                .observeOn(AndroidSchedulers.mainThread())
                .subscribeOn(Schedulers.io())
    ...
    private fun handleResponse(tickets: TicketResponse){
        ...
        adapter.tickets = this.tickets//= TicketAdapter(tickets.result, this)
        recyclerView.adapter = adapter
    ...
    private fun handleResponse2(tickets: TicketResponse){
        Log.i("LateTickets", tickets.result.toString())
        this.tickets = tickets.result.toMutableList()
        adapter.tickets = this.tickets//= TicketAdapter(tickets.result, this)
        recyclerView.adapter = adapter
        adapter.notifyDataSetChanged()
